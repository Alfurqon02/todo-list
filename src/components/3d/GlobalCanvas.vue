...
function renderLoop(time = 0) {
  ...
  // Smooth lerp for continuous journey progress
  currentJourneyProgress += (store.journeyProgress - currentJourneyProgress) * 0.08

  const p = currentJourneyProgress
  const isDesktop = window.innerWidth >= 1024

  // ═════════════════════════════════════════════════════════════
  // CHAINED 3D STATE INTERPOLATION
  // ═════════════════════════════════════════════════════════════
  ...
  if (p < 1.0) {
    ...
    if (t < 0.75) {
      ...
      targetMasterX = isDesktop ? -2.4 * diveT : 0 // moves towards Skills left column
